...
START_TEST(MOVING1) {
  ...
  parser("/home/lunarskii/project/3dViewer/src/GUI/models/cube.obj",
         &testModel);
  ...
  for (int i = 0; i < model.countVertex * 3; i++) {
    // printf("i = %d, model = %f, testModel = %f\n", i, model.vertexCoord[i],
    // testModel.vertexCoord[i]);
...
START_TEST(ROTATION1) {
  ...
  for (int i = 1; i < testModel.countVertex * 3; i += 3) {
    ...
    double y = testModel.vertexCoord[i];

    testModel.vertexCoord[i] =
        cosValue * y - sinValue * testModel.vertexCoord[i + 1];
    testModel.vertexCoord[i + 1] =
        sinValue * y + cosValue * testModel.vertexCoord[i + 1];
  }
...
START_TEST(ROTATION2) {
  ...
  for (int i = 0; i < testModel.countVertex * 3; i += 3) {
    double cosValue = cos(angle);
    double sinValue = sin(angle);
    double x = testModel.vertexCoord[i];

    testModel.vertexCoord[i] =
        cosValue * x + sinValue * testModel.vertexCoord[i + 2];
    testModel.vertexCoord[i + 2] =
        -sinValue * x + cosValue * testModel.vertexCoord[i + 2];
  }
...
START_TEST(ROTATION3) {
  ...
  for (int i = 0; i < testModel.countVertex * 3; i += 3) {
    ...
    testModel.vertexCoord[i] =
        cosValue * x - sinValue * testModel.vertexCoord[i + 1];
    testModel.vertexCoord[i + 1] =
        sinValue * x + cosValue * testModel.vertexCoord[i + 1];
  }
