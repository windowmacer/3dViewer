...
void rotateModel(structRoot *model, double angle, int axis) {
  ...
  for (int i = (axis == X) ? 1 : 0; i < model->countVertex * 3; i += 3) {
    if (axis == X) {
      ...
      model->vertexCoord[i] =
          cosValue * y - sinValue * model->vertexCoord[i + 1];
      model->vertexCoord[i + 1] =
          sinValue * y + cosValue * model->vertexCoord[i + 1];
    } else if (axis == Y) {
      double x = model->vertexCoord[i];

      model->vertexCoord[i] =
          cosValue * x + sinValue * model->vertexCoord[i + 2];
      model->vertexCoord[i + 2] =
          -sinValue * x + cosValue * model->vertexCoord[i + 2];
    } else if (axis == Z) {
      double x = model->vertexCoord[i];

      model->vertexCoord[i] =
          cosValue * x - sinValue * model->vertexCoord[i + 1];
      model->vertexCoord[i + 1] =
          sinValue * x + cosValue * model->vertexCoord[i + 1];
    }
  }
}
