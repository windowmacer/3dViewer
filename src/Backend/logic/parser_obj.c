...
static int isNum(char c) { return c >= '0' && c <= '9'; }
  ...
  for (size_t i = 2; i < strlen(strRead); i++) {
    ...
    if (check_num != 0) {
      ...
      if (check_order == 1) {
        ...
        countIndex++;
        root->vertexIndex =
            realloc(root->vertexIndex, countIndex * sizeof(long));
        if (root->vertexIndex == NULL) {
          root->error = MEMORY_ALLOCATION_ERROR;
          return;
        }
...
static void setCoordToVertexArray(structRoot *root, double *coord_temp,
                                  long countVertex) {
...
static void vertexParser(structRoot *root, char *strRead) {
  ...
  for (size_t i = 2; i < strlen(strRead); i++) {
    ...
    if (num_pos != 0) {
      if (dot_pos != 0) dot_pos = i - dot_pos - 1;
      coord = coord / pow(10, dot_pos);
      if (strRead[num_pos - 1] == '-') coord = -coord;
